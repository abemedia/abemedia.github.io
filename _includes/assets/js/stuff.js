...
$(".box-list>li, .fullrow .row>*").each(function() {
  if (
    $(this)
      .parent()
      .hasClass("box-list-md") ||
    $(this)
      .parent()
      .hasClass("box-list-xs")
  ) {
    $(this).data({
      rotateX: "90deg",
      moveZ: "-400px",
...
if ($(".toc").length > 0) {
  ...
  $.getScript(
    ...
    function() {
      $.getScript(
        "//cdnjs.cloudflare.com/ajax/libs/jquery.tocify/1.7.0/jquery.tocify.min.js",
        tocCallback
      );
    }
  );
    ...
    return $el.css("cursor", opt.cursor).on("mousedown", function(e) {
      ...
      $drag.parents().on("mousemove", function(e) {
        var full_pos = Math.round(e.pageY + pos_y - drg_h),
          minTop = Math.round(
            $drag.parent().offset().top -
              ($drag.height() - $drag.parent().height())
          );
        if (full_pos > $drag.parent().offset().top)
          full_pos = Math.round($drag.parent().offset().top);
...
if ($(".img-link").length > 0) {
  $(".img-link").click(function() {
    $("body").append(
      '<div id="imgModal" class="modal img-modal fade" tabindex="-1" role="dialog" aria-hidden="true"><div class="img thumbnail"><img /><span class="lead"></span></div><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button></div>'
    );
